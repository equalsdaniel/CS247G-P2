...
{
    /// <summary>
    /// Temporary: drops a few evidence entries from both storylines into the
    ...
    public class PlaceholderEvidenceSeeder : MonoBehaviour
    {
        private void Start()
        {
            ...
            log.Collect(new EvidenceEntry(
                "teacher_pawnshop_ticket",
                "Pawnshop Ticket",
                "Found on the body. The financial timeline doesn't match the real Wonjin's known finances.",
                SuspectId.Wonjin));

            log.Collect(new EvidenceEntry(
                "teacher_mira_memory",
                "Mira's Memory Core",
                "A real, unforged argument with \"the tutor\" the night he died. She isn't lying — she just never knew who she was really arguing with.",
                SuspectId.Mira));
        }
    }
}
