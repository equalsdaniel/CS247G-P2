...
export default function Mansion3D({ floor, lang, player, setPlayer, actors, clues, found, onInteract, onOpenPeople, onOpenBoard, peopleLabel, boardLabel }: {
  ...
  return <div className="map-shell three-shell">
    ...
    {isTouchDevice && (
      <div className="touch-controls" aria-label={lang === "zh" ? "移动与转向控制" : "Drive controls"}>
        <TouchDriveButton
          keyName="arrowdown"
          label="▼"
          ariaLabel={lang === "zh" ? "后退" : "Reverse"}
          onPress={pressDirection}
          onRelease={releaseDirection}
        />
        <TouchWheel
          ariaLabel={lang === "zh" ? "方向盘" : "Steering wheel"}
          onChange={(dx) => setLookYawRate(-dx * 2.2)}
          onRelease={() => setLookYawRate(0)}
        />
        <TouchDriveButton
          keyName="arrowup"
          label="▲"
          ariaLabel={lang === "zh" ? "前进" : "Drive forward"}
          onPress={pressDirection}
          onRelease={releaseDirection}
        />
      </div>
...
/**
 * One of the two drive pedals. Same press/hold/release pattern as the
 * keyboard's arrow keys — added to the shared key set on pointer down,
 * removed on up/cancel/leave so dragging a finger off the button doesn't
 * leave it stuck "held."
 */
function TouchDriveButton({ keyName, label, ariaLabel, onPress, onRelease }: {
  keyName: string; label: string; ariaLabel: string;
  onPress: (key: string) => void; onRelease: (key: string) => void;
}) {
  return (
    <button
      className="touch-drive-button"
...
/**
 * A steering-wheel-style rotary drag control: grabbing anywhere on the
 * wheel and dragging left/right reports a normalized [-1,1] turn amount
 * proportional to horizontal drag distance, and springs back to center
 * (calling onRelease) when let go — unlike the drive buttons, there's no
 * "held" state to track between frames, the wheel's rotation angle *is*
 * the current turn rate.
 */
function TouchWheel({ ariaLabel, onChange, onRelease }: {
  ariaLabel: string; onChange: (dx: number) => void; onRelease: () => void;
}) {
  ...
  return (
    <div
      ...
      onContextMenu={(e) => e.preventDefault()}
    >
      <div className="touch-wheel-rim" style={{ transform: `rotate(${angle}deg)` }}>
        <span className="touch-wheel-spoke" />
        <span className="touch-wheel-spoke" />
        <span className="touch-wheel-spoke" />
      </div>
    </div>
